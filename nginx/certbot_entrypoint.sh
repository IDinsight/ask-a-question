...
if [[ -d "$FULLCERT_PATH" ]]; then
    ...
    # Check if 12 hours (43200 seconds) have passed
    if [ $diff_epoch -ge 43200 ]; then
    echo "It's been more than 12 hours since the cert was created."
    echo "Renewing the certificate..."
    certbot renew
    
    else
    hours_left=$(( (43200 - diff_epoch) / 3600 ))
    seconds_left=$(( hours_left * 3600 ))
    echo "The cert was created less than 12 hours ago. Wait for $hours_left hours."
    fi
else
